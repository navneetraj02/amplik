...
const REFRESH_TOKEN = config.google?.refresh_token || "";
const CALENDAR_ID = config.google?.calendar_id || "";
const TEAM_EMAIL = config.team?.email || "";
...
exports.scheduleMeeting = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    if (!CLIENT_ID || !CLIENT_SECRET || !REFRESH_TOKEN || !CALENDAR_ID || !TEAM_EMAIL) {
      return res.status(500).json({ error: "Missing Google configuration in functions.config()" });
    }
    ...
    await gmail.users.messages.send({
      userId: "me",
      requestBody: {
        raw: buildEmail(fromEmail, clientEmail, subjectClient, bodyClient),
      },
    });
    ...
    await gmail.users.messages.send({
      userId: "me",
      requestBody: {
        raw: buildEmail(fromEmail, TEAM_EMAIL, subjectTeam, bodyTeam),
      },
    });
